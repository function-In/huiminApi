...
class MyConnect
{
    ...
    // 选择的数据库
    private $selectdb = "huimin";

    // 连接的数据库
    private $connect;

    // 定义返回数据
    private function setStatus($a, $b, $c)
    {
    ...
    function increase($table, $data)
    {
        if ($this->connect->connect_error)
            return $this->setStatus(0, '数据库连接失败！请联系管理员！', null);
        // 判断数据库表名
        if (empty($table) || $table == '')
            return $this->setStatus(1, "表名未设置或者不正确！", null);
        // 解析需要修改的字段和值
        $_key = "";
        $_val = "";
        if (!is_array($data) || empty($data) || $data == '') {
            return $this->setStatus(2, "数据添加项未设置或者不正确！", null);
        }
        ...
        // 判断状态
        if ($status === true)
            return $this->setStatus(200, "数据添加成功!", null);
        else
            return $this->setStatus(3, $this->connect->error, null);
    }
    /* 
     * 删除数据类函数：
     * 参数：
    ...
    function delete($table, $data)
    {
        ...
        $d1 = '';
        if (!is_array($data) || empty($data) || $data == '') {
            return $this->setStatus(2, "查询条件未设置或者不正确！", null);
        }
        ...
        // 判断是否删除成功
        if ($result === true)
            return $this->setStatus(200, "数据删除成功！", null);
        else
            return $this->setStatus(3, $this->connect->error, null);
    }
    /* 
    ...
    function update($table, $data1, $data2)
    {
        ...
        if (!is_array($data1) || empty($data1) || $data1 == '')
            return $this->setStatus(2, "修改值未设置或者不正确！", null);
        foreach ($data1 as $key => $value)
            $d1 = $d1 . '`' . $key . "`='" . $value . "',";
        // 解析需要修改时的查询条件 -- $data2
        if (!is_array($data2) || empty($data2) || $data2 == '') {
            return $this->setStatus(3, "查询条件未设置或者不正确！", null);
        }
        ...
        // 判断状态信息
        if ($SQLData === true)
            return $this->setStatus(200, "数据修改成功！", null);
        else
            return $this->setStatus(4, $this->connect->error, null);
    }
    ...
    function inquire($table, $data1, $data2)
    {
        // 判断数据库连接状态
        if ($this->connect->connect_error)
            return $this->setStatus(0, '数据库连接失败！请联系管理员！错误信息:\n' . $this->connect->connect_error, null);
        // 判断数据库表名
        if (empty($table) || $table == '')
            return $this->setStatus(1, "表名未设置或者不正确！", null);
        // 判断所需字段名信息或者 SQL 函数
        if ((!is_array($data1) && $data1 != ''))
            return $this->setStatus(2, '查找所需信息必须数组传输！', null);
        $d1 = '';
        ...
        if ($result->num_rows > 0)
            ...
            return $this->setStatus(300, "查询不到任何数据！", $list);
        // 返回数组
        return $this->setStatus(200, "数据查询成功！", $list);
    }


    // 带限制函数查询
    function inquireByLimit($table, $data1, $data2, $limit)
    {
        ...
        // 生成sql语句
        $sql = "select " . (!empty($d1) ? substr_replace($d1, '', -1) : "*") . " from $table " . (!empty($d2) ? "where " . $d2 : "") . ' ' . $limit_sql;
        // 执行查询
        $result = $this->connect->query($sql);
        // 定义新数组
        $list = array();
